...
public class WebSecurityConfiguration__BeanDefinitions {
  ...
  /**
   * Get the bean instance supplier for '(inner bean)#f559c74'.
   */
  private static BeanInstanceSupplier<Filter> getSpringSecurityFilterChainInnerBeanInstanceSupplier(
    ...
    return BeanInstanceSupplier.<Filter>forFactoryMethod(WebSecurityConfiguration.class, "springSecurityFilterChain")
            .withGenerator((registeredBean) -> registeredBean.getBeanFactory().getBean("org.springframework.security.config.annotation.web.configuration.WebSecurityConfiguration", WebSecurityConfiguration.class).springSecurityFilterChain());
  }
